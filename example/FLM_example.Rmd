...
```{r}
#standard_rate <- n ^ -.210
#subtractors <- seq(-.05, 1, length.out = 15)
#bandwidths <- standard_rate + subtractors

bandwidths<-result_rdrobust["bws"][[1]][1,1]+seq(-.2,.2,length.out=10)
bandwidths
results <- WFRD_multiple_bw_alt(Y, X, Z, threshold = 0, bandwidths = bandwidths,  kernel = "Triangular")
```
